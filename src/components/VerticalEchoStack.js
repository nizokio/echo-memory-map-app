...
export default function VerticalEchoStack({ echoes, currentIndex, onIndexChange, onPressCard, onFavoriteToggle }) {
      ...
      <GestureDetector gesture={gesture}>
        <Animated.View style={styles.stackWrapper}>
          {echoes.map((echo, index) => (
            <StackCard key={echo.id} echo={echo} index={index} total={total} stackProgress={stackProgress} isCurrent={index === currentIndex} onPress={() => onPressCard(echo)} onFavoriteToggle={onFavoriteToggle} />
          ))}
        </Animated.View>
      </GestureDetector>
    </View>
  ...
  return (
    <Animated.View style={[styles.cardWrapper, cardStyle]} pointerEvents={isCurrent ? 'auto' : 'none'}>
      <Pressable onPress={onPress} style={styles.cardInner} accessibilityRole="button">
        <Animated.Image source={{ uri: echo.photos[0]?.uri }} style={styles.image} />
        ...
        {isCurrent && <View style={styles.info}>
          <Text style={styles.location}>{echo.location.name}</Text>
          <Text style={styles.name}>{echo.aiMetadata?.title || echo.location.name}</Text>
          <View style={styles.detailRow}><Text style={styles.tag}>{echo.tags[0]}</Text><Text style={styles.details}>{formatDate(echo.capturedAt)} - {echo.photos.length} photo{echo.photos.length === 1 ? '' : 's'}</Text></View>
        </View>}
      </Pressable>
    </Animated.View>
  );
...
const styles = StyleSheet.create({
  ...
  location: { color: '#fff', opacity: 0.85, fontSize: 12, marginBottom: 2 },
  name: { color: '#fff', fontSize: 22, fontWeight: '700', marginBottom: 4 },
  detailRow: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  tag: { color: colors.gold, fontSize: 12, textTransform: 'capitalize' },
  details: { color: '#fff', fontSize: 11 },
});
